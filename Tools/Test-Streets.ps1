param([ValidateSet('Streets','Legacy','Visual','Environment','Police')][string]$Mode='Streets',[switch]$Visible,[switch]$ThirtyFPS,[string]$EvidenceRoot,[string]$PlayerPath)
$ErrorActionPreference='Stop'
$project=Split-Path -Parent $PSScriptRoot
if(-not $PlayerPath){$PlayerPath=Join-Path $project $(if($Mode -eq 'Police'){'Build\PoliceResponse\Funstra.exe'}else{'Build\StreetsWorthFightingFor\Funstra.exe'})}
if(-not $EvidenceRoot){$EvidenceRoot=Join-Path $project ('Evidence\Demo04\'+$Mode.ToLowerInvariant())}
$EvidenceRoot=[IO.Path]::GetFullPath($EvidenceRoot)
New-Item -ItemType Directory -Force -Path $EvidenceRoot | Out-Null
$flag=if($Mode -eq 'Police'){'--police-test'}elseif($Mode -eq 'Environment'){'--environment-test'}elseif($Mode -eq 'Streets'){'--streets-test'}elseif($Mode -eq 'Visual'){'--bandage-visual'}else{'--smoke-test'}
$name=if($Mode -eq 'Police'){'police-runtime-result.txt'}elseif($Mode -eq 'Environment'){'environment-runtime-result.txt'}elseif($Mode -eq 'Streets'){'streets-runtime-result.txt'}elseif($Mode -eq 'Visual'){'bandage-visual-result.txt'}else{'runtime-result.txt'}
